...
TaskType = Enum("TaskType", [("C1", "binary"), ("R", "regression"), ("CM", "multiclass")])
...
class TabrepoAdapter:
    def __init__(self, 
        ...
        self.repo = repo
        datasets = self.repo.datasets(union=True,
                                           problem_type=task_type.value)
        if inst < len(datasets):
            self.dataset = datasets[inst]
        else:
            raise ValueError(f"Tried getting dataset {inst} of {len(datasets)} available")

        # Get all handmade configs for the dataset
        regex = r"_c\d+_"
        self.configs = [
            config_name
            for config_name in self.repo.configs(datasets=[self.dataset],
                                                 union=True)
    ...
    def evaluate(self, config_id: int, seed: Optional[int]=None):
        # TODO: seed
        idx = pd.IndexSlice
        results = self.metrics.loc[idx[:,:,self.configs[config_id]],["metric_error_val", "time_train_s"]]
